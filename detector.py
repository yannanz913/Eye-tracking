...
def detect_pupil(img, pupil_detector):
    img = cv2.imread("/Users/macair/Desktop/VH lab/rat eye.png")
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    cimg = img.copy()
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    img = cv2.medianBlur(img, 5)
    pupil_detector = cv2.HoughCircles(img,cv2.HOUGH_GRADIENT,1,50,
                                param1=50,param2=50,minRadius=0,maxRadius=0)

    pupil_detector = np.uint16(np.around(circles))
    for i in circles[0,:]:
...
def detect_glints(img, glints_detector):
